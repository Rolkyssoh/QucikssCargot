...
export const createNewMission = ({
  ...
  const depatureDateGive =
    selectedDay + ', le ' + `${selectedDate}/${selectedMonth}/${selectedYear}`;
  ...
  return async dispatch => {
    ...
    firestore()
      ...
      .add({
        ...
        depature_day: `${
          missionType == 'programmée' ? depatureDateGive : currentDateMIssion
        }`,
        started_at: '',
        ended_at: '',
      ...
      .then(snapshot => {
        ...
        firestore()
          ...
          .then(snapshot => {
            ...
            {
              baggageImage1 &&
                uploadBaggageImage(baggageImage1, snapshot.path, snapshot.id);
            }
            {
              baggageImage2 &&
                uploadBaggageImage(baggageImage2, snapshot.path, snapshot.id);
            }
            {
              baggageImage3 &&
                uploadBaggageImage(baggageImage3, snapshot.path, snapshot.id);
            }
            {
              baggageImage4 &&
                uploadBaggageImage(baggageImage4, snapshot.path, snapshot.id);
            }
    ...
    firestore()
      ...
      .then(snapshot => {
        ...
        firestore()
          ...
          .then(resp => {
            console.log(
              'response getting Baggage infos in update: ',
              resp.docs[0].id,
            );
            customNavigate('Customer', {
              screen: 'En attente',
          ...
          .catch(error => {
            console.log(
              'error while getting baggage infos in update : ',
              error,
            );
          });
      })
...
const updateBaggageInfos = async (
  theMissinCollectionId,
  missionDocId,
  vol,
  typ,
) => {
