...
const CustomerMissionComponent = props => {
  ...
  const getBaggageInfos = async missionIdCollection => {
    await firestore()
      ...
      .catch(error => {
        console.log('error while getting baggage images : ', error);
      });
  };
  ...
  return (
    <Pressable
      ...
      <View style={styles.view_image_style}>
        {baggageImage && (
          <Image source={{uri: baggageImage}} style={styles.image_style} />
        )}
      </View>
      <View style={styles.view_title_style}>
        {missionItem && (
          <Text style={{fontFamily: 'Nunito-Black'}}>
            {missionItem.mission_title}
          </Text>
        )}
      </View>
    </Pressable>
