...
const MissionInfos = props => {
  ...
  useEffect(() => {
    let isUnmount = false;
    console.log('id for the mission to update: ', props.route.params.missionId);
  ...
  const getMissionInfosForUpdate = async theMissionId => {
    await firestore()
      ...
      .then(result => {
        ...
        console.log(
          'element extrait : ',
          result._data.depature_time.substring(
            0,
            result._data.depature_time.indexOf(':'),
          ),
        );
        console.log(
          'element extrait avec indexof : ',
          result._data.depature_day.indexOf('/'),
        );
        const {
          mission_title,
          mission_destination,
        ...
        props.hoursChanged(
          depature_time.substring(0, depature_time.indexOf(':')),
        );
        props.minutesChanged(
          depature_time.substring(depature_time.lastIndexOf(':') + 1),
        );
        props.startDateChanged(
          depature_day.substring(
            depature_day.lastIndexOf(' '),
            depature_day.indexOf('/'),
          ),
        );
        props.startMonthChanged(
          depature_day.substring(
            depature_day.indexOf('/') + 1,
            depature_day.lastIndexOf('/'),
          ),
        );
        props.startYearChanged(
          depature_day.substring(depature_day.lastIndexOf('/') + 1),
        );
        //baggage infos to update
        getBaggageInfosForUpdate(result.id);
      })
      .catch(error =>
        console.log('error while gettind mission infos for updatin: ', error),
      );
  };
  ...
  const getBaggageInfosForUpdate = async collectionMissionId => {
    firestore()
      ...
      .then(resp => {
        console.log(
          'response getting Baggage infos to update: ',
          resp.docs[0].id,
        );
      ...
      .catch(error => {
        console.log('error while getting baggage infos to update : ', error);
      });
  };

  const getMissionImageForUpdate = async (
    collectionMissionId,
    docMissionId,
  ) => {
    await firestore()
      ...
      .catch(error => {
        console.log('error while getting baggage image once : ', error);
      });
  };
  ...
  return (
    <ScrollView style={{backgroundColor: '#fff'}}>
      <View style={styles.mission_infos_container}>
        <NewMissionHeader
          title={
            props.route.params.missionId
              ? 'Modifier la Mission'
              : 'Infos nouvelle mission'
          }
        ...
        <View style={styles.content_style}>
          <View style={styles.input_view}>
            ...
            />
            <View>
              <Text style={{alignSelf: 'center', fontFamily: 'Nunito-Black'}}>
                Type de la mission
              </Text>
              <RadioButton.Group
                onValueChange={onMissionTypeChange}
                value={props.missionType}>
                <View
                  style={{
                    flexDirection: 'row',
            ...
            <View
              ...
              {props.missionType == 'programmée' && (
                <View style={{alignItems: 'center'}}>
                  ...
                  />
                  <Text style={styles.inputs_styles}>
                    {props.selectedDate}/{props.selectedMonth}/
                    {props.selectedYear}
                  </Text>
                </View>
              )}
              <View style={{alignItems: 'center'}}>
                ...
                />
                <Text>
                  {props.selectedHours}:{props.selectedMinutes}
                </Text>
              </View>
              {show && (
                <DateTimePicker
