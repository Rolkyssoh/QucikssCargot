...
const MissionDetailsScreen = ({navigation, route, idCurrentUser}) => {
  ...
  useEffect(() => {
    ...
    console.log('Dane le détail mission:', route.params.id);
    ...
    firestore()
      ...
      .then(result => {
        ...
        if (result._data.notified_customer || result._data.notified_carrier) {
          navigation.navigate('StartMission', {missionIdForStart: idMission});
        }
      })
  ...
  const countMissionIsOffer = idMission => {
    ...
    firestore()
      ...
      .catch(error =>
        console.log('error while counting carrier is offer: ', error),
      );
  };
  ...
  const getMissionImages = async (missionIdCollection, missionIdDoc) => {
    await firestore()
      ...
      .catch(error => {
        console.log('error while getting Mission images : ', error);
      });
  };
    ...
    <View style={styles.container_details}>
        ...
        <View style={styles.slider_view}>
          <View style={{flex: 3}}>
            <SwiperFlatList
              ...
              showPagination>
              {missionPictures &&
                missionPictures.map(picture => (
                  <View
                    key={picture._data.imageUrl}
                    style={[styles.child, {backgroundColor: 'tomato'}]}>
                    <Image
                      source={{uri: picture._data.imageUrl}}
          ...
          <View
            style={{paddingHorizontal: 10, flex: 1, justifyContent: 'center'}}>
      ...
      {route.params.infosMission && (
        <View style={styles.description_view}>
          <View style={{flexDirection: 'row', justifyContent: 'space-around'}}>
            <View style={{flexDirection: 'row'}}>
              ...
              <Text style={{fontFamily: 'Nunito-Regular'}}>
                {' '}
                {route.params.infosMission.creation_day}{' '}
              </Text>
            </View>
            <View style={{flexDirection: 'row'}}>
              ...
              <Text style={{fontFamily: 'Nunito-Regular'}}>
                {' '}
                {route.params.infosMission.creation_hour}{' '}
              </Text>
            </View>
          </View>
          <View
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <Text style={{fontFamily: 'Nunito-Regular'}}>
                {' '}
                {route.params.infosMission.depature_place}{' '}
              </Text>
            </View>
            <View style={{flexDirection: 'row'}}>
          ...
          <View style={{marginVertical: 25}}>
            <Text
              style={{fontSize: 22, fontFamily: 'Nunito-Black', color: '#fff'}}>
              Description
            </Text>
            <Text style={{fontFamily: 'Nunito-Regular'}}>
              {route.params.infosMission.mission_description}
            </Text>
          </View>
          <View>
            <View style={{flexDirection: 'row', justifyContent: 'center'}}>
      ...
      {route.params.isCustomer && route.params.infosMission.activated != false && (
        <Button
          ...
          type="outline"
          onPress={() =>
            navigation.navigate('OfferReceived', {
...
export default connect(mapStateToProps)(MissionDetailsScreen);
